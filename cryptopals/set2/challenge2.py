import base64
from cryptography.hazmat.primitives.ciphers import Cipher, algorithms, modes

from cryptopals.set1.challenge2 import xor


def aes_cbc_encrypt(msg: bytes, key: bytes, iv: bytes) -> bytearray:
    result = bytearray()
    for i in range(0, len(msg), 16):
        block = msg[i:i+16]
        d = xor(block, iv) if i == 0 else xor(block, result[i-16:i])

        # Regular ECB cipher once the plaintext got XORed
        cipher = Cipher(algorithms.AES(key), modes.ECB())
        encryptor = cipher.encryptor()
        result.extend(encryptor.update(d) + encryptor.finalize())
    return result


def aes_cbc_decrypt(msg: bytes, key: bytes, iv: bytes) -> bytearray:
    result = bytearray()
    for i in range(0, len(msg), 16):
        block = msg[i:i+16]

        # Regular ECB cipher
        cipher = Cipher(algorithms.AES(key), modes.ECB())
        decryptor = cipher.decryptor()
        d = decryptor.update(block) + decryptor.finalize()

        result.extend(xor(d, iv) if i == 0 else xor(d, msg[i-16:i]))
    return result


def main():
    s = """
        
    """

    b = base64.b64decode("".join(map(lambda x: x.strip(), s.split("\n"))))

    key = "YELLOW SUBMARINE".encode("utf-8")
    iv = bytes.fromhex("00000000000000000000000000000000")
    print(aes_cbc_decrypt(b, key, iv))


if __name__ == "__main__":
    main()
